<template>
    ...
    <v-hover v-if="type.name != 'VERTICAL'" v-slot="{ hover }">
    ...
    <v-hover v-else-if="type.name == 'VERTICAL'" v-slot="{ hover }">
        <v-card height="340" width="380" class="ma-4" @click="chooseRouteVertical(line)">
            <v-img class="white--text align-end"  :class="{'escalada':hover}" width="390" height="340" :aspect-ratio="16/9"  :src="index == 0 ? '/img/lineas/tela-vertical.jpg' : '/img/lineas/pvc-vertical.jpg'"  :gradient="hover?'rgba(71, 165, 173, 0.7) 100%, transparent 72px':''"  >
            <template v-slot:placeholder>
                <v-img src="/img/modelos/medium-unavailable.jpg"></v-img>
            </template>
                <v-slide-y-reverse-transition>
                    <div v-if="!hover" class="title d-flex transition-fast-in-fast-out justify-center"  >
                        <h3 class="text-center">{{ index == 0 ? 'TELA VERTICAL' : 'PVC VERTICAL'}}</h3>
                    </div>
                </v-slide-y-reverse-transition>
                <v-slide-x-transition>
                    <div
                    ...
                    >
                        <v-hover v-slot="{ hover }">
                            ...
                            <v-btn @click="chooseRoute(line)" :outlined="!hover" depressed  tile color="white" >
                                {{ index == 0 ? 'TELA VERTICAL' : 'PVC VERTICAL'}}
                            </v-btn>
                          </v-hover>
                    </div>
                </v-slide-x-transition>
...
export default {
    ...
    methods:{
        ...
        chooseRouteVertical(line){
            if(this.type.weaves.length > 0){
                if(this.index == 0){
                    this.$router.replace({name: 'Tejidos', params: {slugLine: line.slug}})
                }
                else if(this.index == 1){
                    this.$router.replace({name: 'Products', params: {slugLine: line.slug, slugWeave: 'pvc-vertical'}});
                }
            }
        }
    }
