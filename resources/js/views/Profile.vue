                ...
                <v-row justify="center" align="center" class="mt-8">
                   <v-avatar :color="getColor" size="150">
                     <span v-if="user.name" class="white--text headline">
                       <div class="d-inline " v-for=" n in user.name.split(' ')" :key="n">
                         {{n.substring(0,1)}}
                        </div>
                    </span>
                  </v-avatar>
                  ...
                  <v-col cols="11" class="mt-8">
                    <v-text-field  prepend-inner-icon="mdi-account" placeholder="Nombre" v-model="user.name" :error-messages="errors.name" outlined></v-text-field> 
                  </v-col>
                  ...
                  <v-col cols="11" >
                    ...
                    <v-col cols="11" v-if="ableToChangePassword" >
                  ...
                  <v-col cols="11" v-else >
                     <v-btn large color="black" class="white--text" block @click="confirmPassword()">Cambiar contraseña</v-btn> 
                      ...
                      <v-col cols="11" >
                        <v-btn @click="editar()" large color="#3ba2a9" class="white--text" block>Guardar
                </v-btn>
                        </v-col>        
        ...
        <v-tab-item>
             ...
             <v-row > 
               ...
               <v-card  height="300"  class="d-flex justify-center mx-3"  @click="$refs.btnUploadImage.click()">
                 <v-icon size="100" v-if="!user.logo">mdi-image
                 </v-icon>
                
                 <v-img v-else
                  class="align-center white--text"
                 :src="(urlTemporal === '') ? (user.logo) : urlTemporal"
              ...
              <v-col cols="12" md="6" sm="12" xs="2">
                <v-text-field  prepend-inner-icon="mdi-map-marker" v-model="user.fiscal_address" placeholder="Dirección Fiscal" :error-messages="errors.fiscal_address" dense :outlined="false" :solo="true" :flat="true"></v-text-field>
              <v-divider  class="mt-n5 mb-3 mx-3" ></v-divider>
                <v-text-field prepend-inner-icon="mdi-truck" v-model="user.shipping_address" placeholder="Direccón de envío" dense :error-messages="errors.shipping_address" :outlined="false" :solo="true" :flat="true"></v-text-field>
              <v-divider  class="mt-n5 mb-3 mx-3" ></v-divider>
                <v-text-field prepend-inner-icon="mdi-whatsapp" v-model="user.movil_number" placeholder="WhatsApp" dense :error-messages="errors.movil_number" :outlined="false" :solo="true" :flat="true"></v-text-field>
               <v-divider  class="mt-n5 mb-3 mx-3" ></v-divider>
                <v-text-field prepend-inner-icon="mdi-phone" v-model="user.phone_number" placeholder="Teléfono fijo" dense :error-messages="errors.phone_number" :outlined="false" :solo="true" :flat="true"></v-text-field>
                 <v-divider  class="mt-n5 mb-3 mx-3" ></v-divider>
                 <v-col>
                     <v-btn @click="editar()" color="#3ba2a9" class="white--text mt-5 mx-n20" block>Guardar
                </v-btn>
                 </v-col>
